...
function correctDiagnosticOffsets(diagnostics, lineOffset, characterOffset) {
...
function formatPart(lines, diagnostics, lineOffset, characterOffset) {
  if (!diagnostics || diagnostics.length === 0) return null;

  // correct line and character offsets by the offsets and indentation in code generated by the template
  const correctedDiagnostics = correctDiagnosticOffsets(diagnostics, lineOffset, characterOffset);
...
class FilterEdit extends LitMvvmElement {
  ...
  formatCodeElement(id, lineOffset, characterOffset) {
    ...
    if (codePart) {
      ...
      const formattedPart = formatPart(codeLines, this.model.diagnostics, lineOffset, characterOffset);
      codeElement.innerHTML = formattedPart || codePart;
    }
    //TODO only set invalid if diagnostics covers these lines!
    if (this.model.diagnostics.length) codeElement.classList.add('invalid');
  ...
  rendered() {
    let codeLineCount = this.formatCodeElement('header', 4, 0);

    let lineOffset = 12 + codeLineCount;
    codeLineCount = this.formatCodeElement('body', lineOffset, 8);

    lineOffset += 20 + codeLineCount;
    codeLineCount = this.formatCodeElement('init', lineOffset, 12);

    lineOffset += 24 + codeLineCount;
    this.formatCodeElement('method', lineOffset, 12);
  }
