...
function correctDiagnosticOffsets(diagMap, lineOffset, characterOffset) {
  const result = diagMap.slice();
  for (let indx = 0; indx < result.length; indx += 1) {
    const dgEntry = result[indx];
    const startPos = dgEntry.span.start;
    startPos.line -= lineOffset;
    startPos.character -= characterOffset;
    const endPos = dgEntry.span.end;
...
function formatPart(lines, diagMap, lineOffset, characterOffset) {
  if (!diagMap?.length) return null;

  // correct line and character offsets by the offsets and indentations in code generated by the template
  const offsetDiagMap = correctDiagnosticOffsets(diagMap, lineOffset, characterOffset);
  const fixedDiagMap = correctSpansForExistingLines(offsetDiagMap, lines);

  const markupSpans = [];
  let nextPosition = { line: 0, character: 0 };
  let nextNewLine = '';

  for (let diagIndx = 0; diagIndx < fixedDiagMap.length; diagIndx += 1) {
    const dgEntry = fixedDiagMap[diagIndx];
    const span = dgEntry.span;
...
class FilterEdit extends LitMvvmElement {
  ...
  formatCodeElement(id, diagMap, lineOffset, characterOffset) {
    const codePart = this.model[id];
    const codeElement = this.renderRoot.getElementById(id);
    let codeLines = [];
    if (codePart) {
      // normalize line-breaks for splitting
      codeLines = codePart.replace(lsrx, '\n').split('\n');
      const formattedPart = formatPart(codeLines, diagMap, lineOffset, characterOffset);
      codeElement.innerHTML = formattedPart || codePart;
    }
    //TODO only set invalid if diagnostics covers these lines!
    if (diagMap.length) codeElement.classList.add('invalid');
    else codeElement.classList.remove('invalid');
  ...
  rendered() {
    ...
    let codeLineCount = this.formatCodeElement('header', diagMap, 4, 0);

    let lineOffset = 12 + codeLineCount;
    codeLineCount = this.formatCodeElement('body', diagMap, lineOffset, 8);

    lineOffset += 20 + codeLineCount;
    codeLineCount = this.formatCodeElement('init', diagMap, lineOffset, 12);

    lineOffset += 24 + codeLineCount;
    this.formatCodeElement('method', diagMap, lineOffset, 12);
  }
