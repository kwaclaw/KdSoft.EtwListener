    ...
    public static class Utils
    {
        ...
        public static List<X509Certificate2> GetClientCertificates(ClientCertOptions certOptions) {
            ...
            if (certOptions.SubjectCN.Length > 0) {
                var clientCerts = CertUtils.GetCertificates(StoreName.My, certOptions.Location, certOptions.SubjectCN, Oids.ClientAuthentication);
                result.AddRange(clientCerts);
            }
            if (certOptions.SubjectRole.Length > 0) {
                var clientCerts = CertUtils.GetCertificates(StoreName.My, certOptions.Location, Oids.ClientAuthentication, crt => {
