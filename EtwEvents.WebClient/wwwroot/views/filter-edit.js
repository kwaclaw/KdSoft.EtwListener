...
function formatFilter(filter, diagnostics) {
  ...
  // correct character offsets by the indentation in the source code generated by the template
  const correctedDiagnostics = diagnostics.slice();
  for (let indx = 0; indx < correctedDiagnostics.length; indx += 1) {
    const dg = correctedDiagnostics[indx];
    dg.lineSpan.start.character -= 12;
    dg.lineSpan.end.character -= 12;
  }
